...
let waybackURL = `https://archive.org/wayback/available?url=${process.env.API_URL.replace('https://', '').replace(/(?<=(.*))\/$/, '')}&timestamp=2020${month}${ (i + 1) < 10 ? ('0' + (i + 1)) : (i + 1) }180000`
...
await axios
  ...
  .then(async (oldApiURLData) => {
    ...
    if(oldApiURLData.data.archived_snapshots === {} || oldApiURLData.data.archived_snapshots === undefined || oldApiURLData.data.archived_snapshots === null || oldApiURLData.data.archived_snapshots.closest === {} || oldApiURLData.data.archived_snapshots.closest === undefined || oldApiURLData.data.archived_snapshots.closest === null) {
...
await axios
  ...
  .then(({ data }) => {
    ...
    console.log('DAY: \n', moment(`2020-${month}-${(i + 1) < 10 ? ('0' + (i + 1)) : (i + 1)}`, 'YYYY-MM-DD').format('YYYY-MM-DD'), '\nPREV:\n', moment(`2020-${month}-${(i + 1) < 10 ? ('0' + (i + 1)) : (i + 1)}`, 'YYYY-MM-DD').subtract(1, 'days').format('YYYY-MM-DD'));
    ...
    // push a new day
    if(covidData['days'].find(el => el.day === moment(`2020-${month}-${(i + 1) < 10 ? ('0' + (i + 1)) : (i + 1)}`, 'YYYY-MM-DD').format('YYYY-MM-DD')) === undefined)
      covidData['days'].push({
