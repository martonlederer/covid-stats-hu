...
export class DataResolver {
  ...
  async allTime() {
    const data: ITotalData = JSON.parse(
      new TextDecoder().decode(
        await fs.readFile(join(__dirname, '../data.json'))
      )
    ).total
    ...
    let apiData: TotalData = {
      ...
      caseRateOthers:
        (data.casesOthers / (population - populationBudapest)) * 100,
  ...
  // days data
  @Query(() => [DayData], {
    description: 'List of daily additional datas and total datas'
  })
  async dailyData(
    @Arg('from', {
      defaultValue: '2020-06-01',
      description: 'The first record to return (date)'
    })
    from: string,
    @Arg('to', {
      defaultValue: moment().format('YYYY-MM-DD'),
      description: 'The last record to return (date)'
    })
    to: string
  ): Promise<DayData[]> {
    if (moment(to).diff(from) < 0)
      throw new Error('From date is more than to date')
    const data: IDayData[] = JSON.parse(
        new TextDecoder().decode(
          await fs.readFile(join(__dirname, '../data.json'))
        )
      ).days,
    ...
    for (const day of data) {
      apiDays.push({
        ...
        total: {
          ...
          caseRateOthers:
            (day.total.casesOthers / (population - populationBudapest)) * 100,
          caseRate:
            ((day.total.casesBp + day.total.casesOthers) / population) * 100,
          recoveryRateBp: (day.total.recoveriesBp / day.total.casesBp) * 100,
          recoveryRateOthers:
            (day.total.recoveriesOthers / day.total.casesOthers) * 100,
          recoveryRate:
            ((day.total.recoveriesBp + day.total.recoveriesOthers) /
              (day.total.casesBp + day.total.casesOthers)) *
            100,
          fatalityRateBp: (day.total.deathsBp / day.total.casesBp) * 100,
          fatalityRateOthers:
            (day.total.deathsOthers / day.total.casesOthers) * 100,
          fatalityRate:
            ((day.total.deathsBp + day.total.deathsOthers) /
              (day.total.casesBp + day.total.casesOthers)) *
            100
        }
      })
  ...
  async getHungaryInfo(): Promise<IHungaryInfo> {
    ...
    try {
      budapestInfo = await axios.get(
        'https://datacommons.org/api/landingpage/data/nuts/HU101'
      )
    } catch (e) {
    ...
    let population = hungaryInfo.data.population,
      populationBudapest =
        budapestInfo.data.highlight.Population.data[0].data.Count_Person,
